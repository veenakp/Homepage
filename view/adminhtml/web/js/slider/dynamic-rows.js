...
define([
    ...
    return uiCollection.extend({
        ...
        processingAddChild: function (ctx, index, prop) {
            ...
            var slider = $('[name = "general[template_type]"]').val();
            var count = 2;
            if (slider == 'sliderbanner_right' || slider == 'sliderbanner_left') {
                count = 1;
            }
            ...
            if (this._elems.length <= count)
            {  
                this.addChild(ctx, index, prop);
            }
            else {
                alert("You can't add more than " +count+ " rows");
            }
        },
